        ...
        <div className="relative w-full h-screen overflow-hidden flex flex-col md:flex-row">
            ...
            <main className="flex-grow h-full overflow-y-auto relative z-10 transition-all duration-300">
                ...
                <div className="px-6 py-8 md:py-16 space-y-12">
                    <div className="bg-gradient-to-r from-gray-50 to-white rounded-3xl p-8 md:p-16 shadow-xl border border-white relative overflow-hidden group">
                        {/* Subtle Overlay for Contrast */}
                        <div className="absolute inset-0 bg-white/30 backdrop-blur-[2px] z-0 pointer-events-none"></div>

                        <div className="relative z-10 max-w-2xl">
                            <span className="inline-block text-gray-500 font-sans font-bold tracking-[0.2em] text-sm mb-4 uppercase">
                                {t('newCollection')}
                            </span>
                            <h2 className="text-5xl md:text-6xl font-serif font-bold text-gray-900 mb-6 leading-tight drop-shadow-sm">
                                {t('heroTitle')}
                            </h2>
                            <p className="text-gray-600 text-lg md:text-xl mb-8 max-w-lg leading-relaxed mix-blend-multiply">
                                {t('heroSubtitle')}
                            </p>
                            <button onClick={() => document.getElementById('products')?.scrollIntoView({ behavior: 'smooth' })} className="bg-gray-900 text-white px-10 py-4 rounded-full font-bold shadow-lg hover:bg-gray-800 hover:shadow-xl transition-all hover:-translate-y-1">
                                {t('viewCollection')}
                            </button>
                        </div>

                        {/* Hero Image */}
                        <div className="absolute right-0 bottom-0 top-0 w-1/2 hidden md:flex items-center justify-end pr-8 pointer-events-none">
                            <img
                                src="https://github.com/despux-net/divina-providentia/blob/main/DIVINA_PROVIDENTIA.png?raw=true"
                                alt="Divina Providentia"
                                className="object-contain h-[95%] w-auto opacity-95 mix-blend-multiply drop-shadow-2xl transition-transform duration-1000 group-hover:scale-105"
                            />
                        </div>

                        {/* Abstract Decorative Circle - Adjusted */}
                        <div className="absolute -left-20 -bottom-40 w-96 h-96 bg-yellow-100/50 rounded-full blur-3xl z-0"></div>
                    </div>
                </div>
